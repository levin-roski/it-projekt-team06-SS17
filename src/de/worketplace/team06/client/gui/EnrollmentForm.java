 ...
 * selektierten Beteiligung. Falls keine selektierte Beteiligung beim
 * Initialisieren übergeben wird, ist das Formular leer, bereit für die
 * Erstellung eines neuen Beteiligunges.
 * 
 * @author Roski
 */
public class EnrollmentForm extends Page {
	 ...
	 * 
	 * @param pToChangeEnrollment Enrollment, die im Formular angezeigt werden soll
	...
	public EnrollmentForm (Enrollment pToChangeEnrollment, final boolean pHeadline) {
		if (pToChangeEnrollment != null) {
			shouldUpdate = true;
			this.toChangeEnrollment = pToChangeEnrollment;
		}
		if (pHeadline) {
			changeHeadline = createHeadline("Beteiligung bearbeiten", true);
			addHeadline = createHeadline("Beteiligung hinzufügen", true);
		}
	}
	/**
	 * Im Konstruktor kann eine selektierte Beteiligung übergeben werden, die
	...
	public EnrollmentForm(Enrollment pToChangeEnrollment, final boolean pHeadline, final Boolean pClosingHeadline) {
		...
		if (pClosingHeadline) {
			...
			addHeadline = createHeadlineWithCloseButton("Beteiligung hinzufügen", true);
		}
		...
		this.add(root);
		/*
		 * Falls eine selektierte Beteiligung übergeben wurde und jetzt dargestellt werden soll
		 */
		if (shouldUpdate) {
			...
			final Button saveButton = new Button("Neue Beteiligung anlegen");
			saveButton.addClickHandler(new ClickHandler() {
				public void onClick(ClickEvent event) {
					if (startDateInput.getText().length() == 0) {
						Window.alert("Bitte vergeben Sie ein Startdatum");
						...
						Window.alert("Bitte vergeben Sie ein Enddatum");
					} else {
						worketplaceAdministration.createEnrollment(startDateInput.getText(), periodInput.getText(), endDateInput.getText(), new AsyncCallback<Enrollment>() {
									public void onFailure(Throwable caught) {
										Window.alert(
